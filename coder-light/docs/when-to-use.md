...
You can move from coder-light to full BMAD when the project grows up. The plan format (`plans/YYYY-MM-DD-<slug>.md`) and the skill format are compatible. Install the method skills via `pnpm dlx skills add bmad-code-org/BMAD-METHOD` and start producing PRDs / stories alongside the existing work. coder-light's skills don't conflict with full BMAD's.
